...
(defvar line-sep " | ")
(defvar line-date "%A, %d %B")
(defvar line-time "%H:%M")
(defvar line-volume-icon " ")
(defvar line-telega-icon " ")
...
(defun pre-line ()
  (make-string (/
                (- (frame-text-cols) (string-width (pre-line-str)))
                2)
               ? ))
...
(defun bk/workspace-init ()
  (let ((workspaces '("Dev" "IDEA" "Net" "Dotfiles" "Chats" "Media"))
        (index 1))
    (dolist (workspace workspaces)
      ...
      (if (equal index 5) (telega))
      (if (equal index 6) (dired "/run/media/solus/hdd"))
      (setq index (+ index 1)))
    (exwm-workspace-switch-create 4)
    (bk/switch-to-dashboard)))

(defun bk/run-shell-commands ()
  (let ((commands '("feh --bg-fill /run/media/solus/hdd/Pictures/wallpapers/27.jpg"
