...
#+begin_src emacs-lisp
(defun bk/disk-avail-size (disk)
    (let ((output (shell-command-to-string (format "df -h %s --output=avail" disk)))
          (regex "\\([0-9]+G\\)"))
      (save-match-data
...
#+begin_src emacs-lisp
(defun bk/get-used-mem ()
  (let ((proc (async-start
               (lambda ()
                 (replace-regexp-in-string "r?\n$" ""
                                           (shell-command-to-string "free -h --si | grep Mem | awk '{print $3}'"))))))
    (async-get proc)))
#+end_src
...
;;   (let ((msg_count (plist-get telega--unread-message-count :unread_count))
;;         (msg_unmuted_count (plist-get telega--unread-message-count :unread_unmuted_count))
;;         (chat_count (plist-get telega--unread-chat-count :unread_count))
;;         (chat_unmuted_count (plist-get telega--unread-chat-count :unread_unmuted_count)))
;;     (format "m: %s(%s) u: %s(%s)"
;;             (- chat_count chat_unmuted_count) (- msg_count msg_unmuted_count)
...
(add-hook 'telega-root-update-hook 'minibuffer-line--update)
#+end_src

** minibuffer-line

- =Display system info in minibuffer line mode=

#+begin_src emacs-lisp
(straight-use-package 'minibuffer-line)

;; (setq-default minibuffer-line-format '((:eval
;;                                         (let ((sep (propertize
;;                                                     (format " %s " "|")
;;                                                     'face '(:foreground "dim gray" :background "#282c34")))
;;                                               (date (propertize
;;                                                      (format-time-string "%A, %d %B")
;;                                                      'face '(:foreground "gray" :background "#282c34")))
;;                                               (time (propertize
;;                                                      (format-time-string "%H:%M")
;;                                                      'face '(:foreground "gray" :background "#282c34")))
;;                                               (volume-icon (propertize
;;                                                             (format "%s" "")
;;                                                             'face '(:foreground "gray" :background "#282c34" :height 110)))
;;                                               (volume (propertize
;;                                                        (format " %s " (bk/get-current-volume))
;;                                                        'face '(:foreground "gray" :background "#282c34")))
;;                                               (telega-icon (propertize
;;                                                             (format "%s" "")
;;                                                             'face '(:foreground "gray" :background "#282c34" :height 115)))
;;                                               (telega-msg-count (propertize
;;                                                                  (format " %s" (bk/telega-msg-count))
;;                                                                  'face '(:foreground "gray" :background "#282c34")))
;;                                               (persp (propertize
...
;;                                                                (string-width (concat sep date sep time sep volume-icon volume sep telega-icon telega-msg-count sep persp sep)))
;;                                                             2)
;;                                                          ? ) 'face '(:background "#282c34"))
;;                                            sep date sep time sep volume-icon volume sep telega-icon telega-msg-count sep persp sep)))))

;; (setq-default minibuffer-line-format
;;               '((:eval
;;                  (let ((sep (format " %s " "|"))
;;                        (date (format-time-string "%A, %d %B"))
;;                        (time (format-time-string "%H:%M"))
;;                        (volume-icon (format "%s" ""))
;;                        (volume (format " %s " (bk/get-current-volume)))
;;                        (telega-icon (format "%s" ""))
;;                        (telega-msg-count (format " %s" (bk/telega-msg-count)))
;;                        (persp (format "%s" (persp-current-name))))
;;                    (concat
;;                     (make-string (/ (- (frame-text-cols)
;;                                        (string-width
;;                                         (concat
;;                                          sep date sep time sep volume-icon volume
;;                                          sep telega-icon telega-msg-count sep persp sep)))
;;                                     2)
;;                                  ? )
;;                     sep date sep time sep volume-icon volume sep telega-icon telega-msg-count sep persp sep)))))
...
(defun bk/info ()
  (let ((sep
        ...
        (persp
         (propertize
          (format "%s" (persp-current-name))
          'face '(:foreground "white" :background "#282c34"))))
  (concat sep date sep time sep volume-icon volume sep telega-icon telega-msg-count sep persp sep)))

(setq-default minibuffer-line-format
              '((:eval
                 (propertize
                  (make-string (/ (- (frame-text-cols) (string-width (bk/info))) 2) ? )
                  'face '(:background "#282c34")))
                (:eval (bk/info))))
...
;;                   (format "%s " "|")
;;                   'face
;;                   '(:foreground "dim gray" :background "#282c34")))
;;                 (:eval
;;                  (propertize
;;                   (format-time-string "%A, %d %B")
;;                   'face
;;                   '(:foreground "gray" :background "#282c34")))
;;                 (:eval
;;                  (propertize
;;                   (format " %s " "|")
;;                   'face
...
;;                   (format "%s" "")
;;                   'face
;;                   '(:foreground "gray" :background "#282c34" :height 110)))
;;                 (:eval
;;                  (propertize
;;                   (format " %s " (bk/get-current-volume))
;;                   'face
...
;;                   (format "%s" "")
;;                   'face
;;                   '(:foreground "gray" :background "#282c34" :height 115)))
;;                 (:eval
;;                  (propertize
;;                   (format " %s" (bk/telega-msg-count))
;;                   'face
...
(defun bk/notify-squats ()
  (let ((count 1))
    (while t
      (sit-for 3600)
      (notifications-notify :title (concat "Do Squads!!!" " " (number-to-string count) "th set"))
      (setq count (1+ count)))))
#+end_src
...
(defun bk/workspace-init ()
  (let ((workspaces '("Dev" "IDEA" "Net" "Dotfiles" "Media" "Chats"))
        (index 1))
